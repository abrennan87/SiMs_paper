...
In this paper we have recast constraints from ATLAS missing energy searches onto a set of three simplified models. We compare the constraints from searches for monojet + \met, $Z(\rightarrow \bar l l) + \met$, and $W^\pm/Z (\rightarrow$ hadrons) + \met, finding that ...

Rather than placing a constraint in the $\Mmed - \mDM$ plane for a fixed value of the coupling strength, we constrain the coupling strength as a function of both $\Mmed$ and $\mDM$ in a 3D plane. Whilst this does necessitate the introduction of some approximations, it allows us to see the full dependence of the DM production cross-section on the free parameters of the models.

We also compare these constraints to constraints from direct detection and the relic abundance. Although each search has a different set of assumptions, this allows us to see that each technique is complementary and offers strong constraints in certain regions of parameter space.
