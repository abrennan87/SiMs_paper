...
\hspace{1cm}\st{The monojet contraints are derived from a Supersymmetry search conducted by the ATLAS Collaboration (see Section X).} \comm{Change this to Exotics monojet.} Similarly, the mono-Z constraints are derived from a dark matter search originally optimised for the D1, D5 and D9 effective operators (see Section \ref{monoZ_constraints}). \textcolor{magenta}{Lastly, the mono-W constraints are derived from...} \comm{We should put references to all the papers here! And also include something like 'These analyses will be described in further detail in Section X.' - Amelia}
\bigskip
