...
For the mono-jet channel, matching between \MG$\mbox{ }$and \PYTHIA is performed using the MLM scheme. A proper implementation of this scheme would require the creation of multiple subsamples per mass and coupling point, with each sample possessing a different matching scale (QCUT). The matching scales are collectively defined to ensure that the kinematics of both off- and on-shell mediator production are well simulated and that sufficient events are created to survive the ATLAS $\met$ trigger system \question{or to probe the high $\met$ regime?}. To prevent double-counting, the subsamples would then be merged according to the $p_{T}$ of the leading parton in each event. Indeed, the ATLAS mono-jet analysis (discussed in Section \ref{monojet_constraints}) uses MC samples which consist of events from a QCUT = 80 (300) GeV subsample with a leading jet with $p_{T} <$ 350 GeV ($p_{T} \geq$ 350 GeV).
