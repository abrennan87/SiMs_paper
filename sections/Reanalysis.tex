  ...
  \begin{center}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
      ...
      \begin{tikzpicture}
        ...
        \draw[photon] (-1,2.25) -- (0,3)node[right]{$W/Z$};
    ...
    \begin{subfigure}[b]{0.45\textwidth}
      ...
      \begin{tikzpicture}
        ...
        \draw[photon] (0,0.25) -- (2, 0.25)node[right]{$W$};
    ...
    \caption{Representative dark matter pair-production processes with a parton or a $W$ or $Z$ boson in the final state for the $s$-channel (a,b) and $t$-channel (c,d) models.}
...
% A proper implementation of this scheme would require the creation of multiple subsamples per mass and coupling point, with each subsample possessing a different matching scale (QCUT). The specific values taken by the QCUT are chosen to ensure that the kinematics of both off- and on-shell mediator production are well simulated for each masspoint and that sufficient events are created to survive the ATLAS $\met$ trigger \question{or to probe the high $\met$ regime?}. To prevent double-counting, the subsamples are then merged according to the $p_{T}$ of the leading parton in each event. Indeed, the ATLAS mono-jet analysis (discussed in Section \ref{monojet_constraints}) uses MC samples which consist of events from a QCUT = 80 (300) GeV subsample with a leading jet with $p_{T} <$ 350 GeV ($p_{T} \geq$ 350 GeV).
%The systematic uncertainty associated with this matching procedure is then estimated as described in Ref. \cite{CERN-THESIS-2015-038}.

%For this analysis, we instead perform parton matching between \MG$\mbox{ }$and \PYTHIA using only a single matching scale of 80 GeV \comm{per mass and coupling point}. This approach has the advantage of substantially reducing both the complexity and computational expense of the mono-jet channel MC generation and systematic uncertainty estimation procedures. While the specific matching scale of 80 GeV ensures that we have enough statistics to derive physical limits for low-$m_{\chi}$/low-$\Mmed$ masspoints, it necessarily equates to an increased uncertainty on the higher-$\Mmed$ limits (due to mismodelling of the $\met$ distribution). We deem this trade-off acceptable however as the 80 GeV, single-QCUT approach has been shown to adequately reproduce the results of the ATLAS mono-jet analysis for the masspoints of interest (see Section \ref{monojet_validation}).
%For this analysis, we instead perform parton matching between \MG$\mbox{ }$and \PYTHIA using only a single matching scale per mass and coupling point. Specifically, for mediator masses below 320 GeV we use a matching scale of 80 GeV while for masspoints above the 320 GeV threshold we use a matching scale of $\Mmed/4$. In analogy with the logic above, the 80 GeV matching scale ensures that we have enough statistics to derive physical limits for low-$m_{\chi}$/low-$\Mmed$ masspoints. The $m_{M}/4$ matching scale then ensures that $m_{\chi}$/high-$\Mmed$ limits aren't dominated by mismodelled $\met$ uncertainties. This two-stage approach has been shown to adequately reproduce the results of the ATLAS mono-jet analysis for the masspoints of interest (see Section \ref{monojet_validation}). In addition, this approach substantially reduces both the complexity and computational expense of the mono-jet channel MC generation and systematic uncertainty estimation procedures.
...
%\subsubsection{Mass and Coupling Points} % I moved this to earlier in the paper, as the width discussion kind of refers to the masses used.
...
\begin{tabular}{c|c|c}
 ...
 \hline
\end{tabular}
\caption{The ATLAS mono-jet $\met$ signal regions and \comm{associated} observed (expected) model-independent upper limits on $\sigma \times \mathcal{A} \times \epsilon$ at 95\% confidence level. Adapted from Ref. \cite{Aad:2015zva}.
...
\comm{Note to Johanna: Here you should discuss the original intention of the mono-WZ analysis. Also comment on any validation you did in order to confirm that you could use the results of the analysis - the bulk of this should go into the mono-WZ section of App. A. Lastly, discuss or list the cuts used in the analysis and the uncertainties associated with the results. See mono-jet and mono-Z sections above.}
