...
In this paper, monojet constraints are derived from a search for new phenomena conducted by the ATLAS Collaboration using $pp$ collisions at $\sqrt{s}=$ 8 TeV as described in Ref. \cite{Aad:2015zva}. Similarly, the mono-$Z$ constraints are derived from an ATLAS dark matter search originally optimised for the D1, D5 and D9 effective operators (see \cite{Aad:2014monoZlep}). These analyses are described in further detail in Sections \ref{monojet_constraints} and \ref{monoZ_constraints} respectively.
...
\draw[fermion] (2.75,0.75) -- (3.5,1.5)node[right]{$\chi$};
\draw[antifermion] (2.75,0.75) --(3.5,0)node[right]{$\bar{\chi}$};
\draw[fill] (2,0) circle [radius=0.0]node[below]{$\mbox{ }g_{q}$};
...
\draw[antifermion] (2.75,0.75) --(3.5,0)node[right]{$\bar{\chi}$};
\draw[fill] (2,0) circle [radius=0.0]node[below]{$\mbox{ }g_{q \chi}$};
\draw[fermion] (2,0) --(3.5,-1.5)node[right]{$\chi$};
\draw[fill] (2.75,0.75) circle [radius=0.0]node[right]{$\mbox{\textcolor{white}{\tiny m}}g_{q \chi}$};
\end{tikzpicture}
\caption{}
