...
\hspace{1cm}The uncertainties associated with the aforementioned parameters \st{were} are estimated in the \st{usual} \fg{following} way. Firstly, the factorisation and renormalisation default scales \st{were} are varied \st{simultaneously} together by a factor of 0.5 \fg{(`down')} and of 2 \fg{(`up')} \st{times the default scale}. The uncertainty on the final \st{acceptance} \fg{cross section limit}, $\cancel{\mathcal{A}}$ \fg{$\sigma_{\mathrm{lim}}$}, was then taken to be the average change in $\cancel{\mathcal{A}}$ \fg{$\sigma_{\mathrm{lim}}$} resulting from these up and down variations. \textcolor{magenta}{Similarly, the uncertainty on $\mathcal{A}$ associated with $\alpha_{s}$ and the PDF was estimated by...}
...
\begin{tabular}{c|c|c|c}
 \hline
 \hline
 Signal Region & Minimum $E_{T}^{miss}$ [GeV] & $N_{obs}$ & $N_{exp}$ \\ %$N_{obs}/\mathcal{L}$ & $N_{exp}\mathcal{L}$ \\
 \hline
...
The signal selection and Monte Carlo (MC) generation procedures for the monojet channel were validated via reproduction of the ATLAS limits on the suppression scale, M$_{*}$, for the s-channel vector simplified model (refered to as the Z' model in the ATLAS monojet analysis). The ATLAS and reproduced 95\% confidence level (CL) limits on M$_{*}$ for the case where $m_{\chi} = $ 50 GeV, $\Gamma = M/8\pi$ and $\sqrt{g_{q}g_{\chi}} = 1$ are shown in Table \ref{M_star_limits_monojet}. Note that the slight difference between the ATLAS and reproduced limits is expected given that the Monte Carlo generation procedure employed in this analysis (see Section \ref{signal_generation}) does not include a full simulation of the ATLAS detector. \comm{Instead, reconstruction effects are modelled by smearing the leading jet $p_{T}$ distribution.} \comm{Make a comment about the treatment of CLs?}
...
In keeping with the ATLAS analysis, monojet signal samples are generated using the MLM scheme for matching between \MG and \PYTHIA. However, while the ATLAS analysis favours a single matching scale (QCUT) of 80 GeV, our approach is to use a sliding value of $m_{\chi}/4$. \comm{This ensures that all jet-related kinematic distributions are smoothly connected across all dark matter masspoints.} \draft{Also note that while the signal samples used for validation correspond to the processes $pp \rightarrow j\chi\bar{\chi}$ and $pp \rightarrow jj\chi\bar{\chi}$ where $j$ is a final state jet, constraints on the simplified models discussed in Section \ref{SiM_models} are determined using signal samples with zero, one and two jets in the final state.} \comm{This small deviation from the ATLAS analysis is motivated by the observation that the limits on $M_{*}$ improve with the addition of the process $pp \rightarrow \chi\bar{\chi}$.}
...
\begin{tabular}{c|c|c|c}
 \hline
 \hline
 $M$ [GeV] & ATLAS M$_{*}$ [GeV] & Reproduced M$_{*}$ [GeV] & Difference [\%] \\ 
 \hline
50 & & & \\
100 & & & \\
300 & & & \\
...
12000 & & & \\
 ...
 \hline
\end{tabular}
\caption{Comparison of the 95\% CL limits on the suppression scale, M$_{*}$, for the s-channel vector mediator model for the case where $m_{\chi} = $ 50 GeV, $\Gamma = M/8\pi$, $\sqrt{g_{q}g_{\chi}} = 1$ and QCUT = 80 GeV. Adapted from Ref. \cite{Aad:2015zva}.}
\label{M_star_limits_monojet}
\end{table}
\bigskip

\comm{An example of blah is shown in Table blah. Comment on inevitable discrepancies between ATLAS values and mine and how this reflects the inadequacies of our detector simulation. Also make a comment about why it's still okay to use our results, etc. MM}
