...
\hspace{1cm}The uncertainties associated with the aforementioned parameters are estimated in the following way. Firstly, the factorisation and renormalisation default scales are varied together by a factor of 0.5 \fg{(`down')} and by a factor of 2 \fg{(`up')} \st{times the default scale}. The uncertainty on the final \st{acceptance} \fg{cross section limit}, $\cancel{\mathcal{A}}$ \fg{$\sigma_{\mathrm{lim}}$}, is then taken to be the average change in $\cancel{\mathcal{A}}$ \fg{$\sigma_{\mathrm{lim}}$} resulting from these up and down variations. \textcolor{magenta}{Similarly, the uncertainty on $\mathcal{A}$ associated with $\alpha_{s}$ and the PDF is estimated by...}
...
%%%% Note: I say 2%, because for the cross sections that we could take from the paper (fig 7), which are for m_eta = 1000, we get differences of 1.1%, 2% and -1.7%. %%%
