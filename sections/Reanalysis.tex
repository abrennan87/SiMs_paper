...
We now move to describe the two ATLAS analyses that are used for re-interpretation in this work.
...
\begin{flushleft}
The ATLAS mono-jet plus missing transverse energy search \cite{Aad:2015zva} was originally designed to set limits on three new physics scenarios\comm{,} \st{: the production of light grativinos in association with gluinos or scalar quarks in a gauge-mediated supersymmetric model, the production of graviton modes in the Arkani-Hamed, Dimopoulos, and Dvali model for large extra spatial dimensions, and} \comm{the most relevant of which was} the production of WIMP DM within the context of several effective operators. The analysis also includes a brief study of the $s$-channel vector mediator simplified model (therein referred to as the $Z'$ model).
\bigskip

For the mono-jet channel, signal selection is \st{carried out as described in the ATLAS mono-jet analysis} as follows (\comm{or, `based on at least one hard jet recoiling against missing energy'}): \st{initially, events are required to have $\met>$ 150 GeV and at least one jet with $p_{T} >$ 30 GeV and $|\eta| <$ 4.5} (\comm{My logic here is that these cuts are for trigger, or pre-selection - they are overwritten by later cuts, right? So I think not worth including.}). The background contribution from QCD jets (\comm{Is this the dominant background? If so, include `dominant', if not, this cut is maybe better understood as implementing back-to-backness.}) is managed with the requirement that all jets (\comm{Is there an upper limit on $N_{jets}$?}) passing this initial selection criteria also satisfy $|\Delta\phi(\mathrm{jet},\metvec)|>1.0$. In order to isolate monojet-like topologies (\comm{This line makes more sense if the mono-jet-like topology is defined above, ie back-to-back jet + MET.}), surviving events are then required to have a leading jet, $j_{1}$, with $p_{T} >$ 120 GeV and $|\eta| <$ 2.0 satisfying $p_{T}^{j_{1}}/\met >$ 0.5. (\comm{Is there a lepton veto here? Is it in the code?}) Lastly, nine separate signal regions are defined with increasing lower thresholds on $\met$, which range from 150 GeV to 700 GeV as shown in Table \ref{monojet_SRs}.
\bigskip
