...
The signal selection and Monte Carlo generation procedures for the monojet channel were validated via reproduction of the ATLAS limits on the suppression scale, M$_{*}$, for the s-channel vector simplified model (refered to as the Z' model in the ATLAS monojet analysis). \comm{An example of blah is shown in Table blah. Comment on inevitable discrepancies between ATLAS values and mine and how this reflects the inadequacies of our detector simulation. Also make a comment about why it's still okay to use our results, etc. MM}
